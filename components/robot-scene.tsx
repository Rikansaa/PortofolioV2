...
function RobotPrototype({
  ...
  useFrame((state, delta) => {
    ...
    const tx = globalPointerRef.current.x;
    const ty = globalPointerRef.current.y;
...
export function RobotSceneOnly({
  color = "#e4e4e7",
  scale = 1,
  ...
  RobotHeroProps,
  "color" | "scale" | "pantallaColor" | "pantallaBrillo" | "blinkCycle" | "metalness"
> = {}) {
  const entorno = {
    luzAmbiente: 0.9,
    luzPrincipal: 0.6,
  ...
  return (
    <div className="h-[420px] w-full sm:h-[480px] lg:h-[560px]">
      <Canvas shadows camera={{ position: [0, 0.2, 6], fov: 40 }}>
        <ambientLight intensity={entorno.luzAmbiente} color="#ffffff" />
        <directionalLight
          position={[0, 6, 3]}
